...
class EmbeddingModel(tf.keras.Model):
    ...
    def __init__(self, config: BaseConfig, tower_type: str, vocab_dict: dict, adapted_layers: Dict[str, tf.keras.layers.Layer]):
        ...
        self._all_features = (self.str_lookup_features + self.int_lookup_features + 
                             self.normalizer_features + self.str_features + self.list_features)
        self._embeddings = {}
                ...
                self._embeddings[feature] = tf.keras.Sequential(
                    [
                        ...
                        tf.keras.layers.Embedding(
                            ...
                            output_dim=self.config.layer_output,
                            mask_zero=True,
                            name=feature + "_emb"
                        )
                    ],
    ...
    def call(self, features: Dict[str, tf.Tensor]) -> tf.Tensor:
        ...
        for feature in self._all_features:
            ...
            elif feature in self.str_lookup_features or feature in self.int_lookup_features:
        ...
        if embeddings:
            ...
            return tf.zeros((tf.shape(list(features.values())[0])[0], 1), dtype=tf.float32)
